...
public class Send_File implements Runnable
{
	...
	public Send_File(Socket socket, String data, Main_Window mainwindow)
	{
	...
	public void run()
	{
		try
		{
			...
			Send();// 发送文件
			socket.close();
			new Thread(new Ask_File_List(Username, GetFolder(RemoteRoute))).start();
		} catch (Exception e)
		{
	...
	public void Send()
	{
		try
		{
			...
			while ((length = fin.read(SendBuf, 0, SendBuf.length)) != -1)
			{
				...
				SendBuf = AES_ECB.encrypt(SendBuf, cKey);//rc.rc4(SendBuf, "123");
